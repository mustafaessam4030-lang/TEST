...
const EQUIP=(()=>{
  ...
  const STEP_LABEL={
    LEARN_LAYOUT   :{en:'First run: learning the SIS page layout',
                     ar:'أول مرة: بتعلّم شكل صفحات SIS'},
  ...
  let llmReadyP=null;
  function llmReady(){
    if(!llmReadyP)llmReadyP=call('/v1/llm/status',{timeoutMs:4000})
      .then(r=>!!(r&&r.httpStatus===200&&r.payload&&r.payload.configured))
      .catch(()=>false);
    return llmReadyP;
  }
  ...
  async function maybeLookup(raw,ents,cls,L,opts){
    ...
    if(state){
      ...
      // Not an equipment turn: the general assistant answers, and the machine
      // we were discussing stays in its context (`current` is left alone).
      if(mode==='PASS')return null;
      // Small talk: with the model connected, let it answer naturally — it
      // still sees the current record. Without it, the brain's own reply.
      if(mode==='CHAT'&&await llmReady())return null;
      if(mode==='CHAT'||mode==='REFUSE'){
        return {ok:false,clarify:true,conversational:true,mode:mode,
                message:state.message,suggestions:state.suggestions||[],state:state};
      }
...
- Do not speculate about what the machine might be.`;
    ...
    if(r.ok){
      return `EQUIPMENT RECORD (authoritative — retrieved by tool in this conversation; the machine being discussed)
...
- Do NOT state any equipment attribute for this serial. You do not have one.`;
  ...
  function mergeAnswer(out,r,L){
    ...
    const ar=L==='ar';
    if(r.conversational){
      // A reply the brain decided on (thanks, hello, a refusal). Certain, and
      // it claims nothing about any machine.
      out.reply=r.message||out.reply;
      out.confidence=0.9;
      if((r.suggestions||[]).length)out.quick_replies=r.suggestions.slice(0,4);
      return out;
    }
    if(r.clarify){

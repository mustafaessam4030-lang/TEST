"""Maia's tool layer: JSON definitions + a deterministic HTTP dispatcher.
...
TOOLS_PATH = Path(__file__).resolve().parents[1] / "schemas" / "maia_tools.json"
SERIAL_RE = re.compile(r"^[A-Z0-9]{3,17}$")
API_BASE = os.environ.get("MAIA_API_BASE", "http://localhost:8080")
...
class MaiaToolDispatcher:
    def __init__(self, *, api_base: str = API_BASE, actor: str = "maia-agent",
                 scopes: str = "equipment.read equipment.search",
                 timeout_s: float = 45.0) -> None:
        self.api_base = api_base.rstrip("/")
        self.headers = {"x-maia-actor": actor, "x-maia-scopes": scopes}

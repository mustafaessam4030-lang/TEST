...
open the chat and ask Maia for that serial.

## Prerequisites

```bash
git clone <repo> && cd maia
...
4. Set `MAIA_ALLOW_LIVE_AUTOMATION=true`.
...
python scripts/capture/capture_selectors.py --auto-login \
    --secret-ref vault://kv/maia/cat_sis            # VAULT_ADDR + VAULT_TOKEN in the env
python scripts/capture/capture_selectors.py --auto-login \
    --secret-ref env://MAIA_CAT_SIS                 # MAIA_CAT_SIS_USERNAME / _PASSWORD
```
...
| `--auto-login` | authenticates from `MAIA_CAT_SIS_USERNAME`/`PASSWORD`, then inventories the authenticated DOM and **stops** | a shared runner where a human cannot click; it never picks selectors for you |

...
```
User      Maia(LLM)     maia-api        Freshness   Repo(Snowflake)   Worker/Playwright   SIS2
...
```
User → Maia → get_equipment_from_database("SN123456")
...
Maia answers from the store, states "من الـ internal data store، آخر سحب من SIS بتاريخ …".
...
Maia → search_equipment_in_sis(serial, reason="STALE_REFRESH")
...
Maia: "مقدرتش أوصل لـ SIS دلوقتي (Timeout). عندي نسخة محفوظة عمرها 210 يوم — أعرضها؟"
...
Maia: "السيريال SN123456 مش موجود في Caterpillar SIS. اتأكد من الرقم أو ابعتلي الـ PIN كامل."
...
Maia: "فيه تحقق أمني على SIS محتاج تدخل بشري. فتحت تذكرة للفريق (Run ID …)."

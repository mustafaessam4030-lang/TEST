...
So even a manipulated page cannot put arbitrary values into a Maia answer.
...
**Then** the prompt layer: Maia must state source + timestamp + run id, must use
...
`SOURCE_REGISTRY`, add a normalizer mapping. No change to Maia, to her tools, to
...
visible and reportable rather than overwritten — Maia can honestly say "ERP says
...
precedence — while the tool surface Maia sees stays five tools.

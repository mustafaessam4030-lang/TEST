"""Maya's orchestration loop (server-side reference implementation).
...
from agent.tools import MayaToolDispatcher, load_tools

logger = logging.getLogger(__name__)

MODEL = os.environ.get("MAYA_AGENT_MODEL", "claude-opus-5")
...
class MayaAgent:
    def __init__(self, *, api_base: str | None = None, actor: str = "maya-agent") -> None:
        self.client = anthropic.AsyncAnthropic()
        self.dispatcher = MayaToolDispatcher(
            api_base=api_base or os.environ.get("MAYA_API_BASE", "http://localhost:8080"),
            actor=actor)
        ...
        while True:
            ...
            for block in blocks:
                ...
                if tool_calls > MAX_TOOL_CALLS_PER_TURN or sis_calls > MAX_SIS_CALLS_PER_TURN:
                    ...
                    logger.info("maya.tool", extra={"extra_fields": audit[-1]})

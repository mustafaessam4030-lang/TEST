<#
    Start the whole stack on this PC:
      Maya chat (browser)  →  tool gateway  →  Playwright worker  →  CAT SIS
...
Write-Host "`nStarting the gateway, the worker and the Maya UI..." -ForegroundColor Cyan
Start-Process "http://127.0.0.1:$UiPort/maya.html"
python @args

...
def check_credentials() -> None:
    ...
    ref = os.environ.get("MAYA_SIS_SECRET_REF") or (cfg.get("auth") or {}).get("secret_ref", "")
    try:
        ...
        add(FAIL, "SIS credentials", f"{ref}: {exc.message}",
            "export MAYA_SIS_SECRET_REF=vault://kv/maya/cat_sis (with VAULT_ADDR/VAULT_TOKEN), "
            "or env://MAYA_CAT_SIS with MAYA_CAT_SIS_USERNAME / _PASSWORD set")
...
def check_switch() -> None:
    on = os.environ.get("MAYA_ALLOW_LIVE_AUTOMATION", "").lower() == "true"
    add(OK if on else WARN, "live automation switch",
        "enabled" if on else "off (make e2e turns it on for you)",
        "" if on else "export MAYA_ALLOW_LIVE_AUTOMATION=true for a manual run")
...
async def check_browser_reach() -> None:
    ...
    if os.environ.get("MAYA_CHROMIUM_PATH"):
        launch["executable_path"] = os.environ["MAYA_CHROMIUM_PATH"]
    try:
        ...
        add(FAIL, "browser launches", str(exc)[:140],
            "python -m playwright install chromium, or set MAYA_CHROMIUM_PATH")


def main() -> int:
    print("\n  MAYA — real-SIS readiness\n" + "  " + "─" * 66)
    check_playwright()
    check_display()
    check_credentials()
    check_selectors()

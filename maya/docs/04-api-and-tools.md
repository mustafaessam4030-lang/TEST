# 04 — API Design & Maya Tool Contracts
...
* There is **no** response shape that returns equipment fields without
  `source` + `retrieved_at`. Maya literally cannot render an unattributed value.
...
## 4.2 Maya's five tools

Definitions live in `schemas/maya_tools.json` (Claude `tools` array, `strict: true`,
...
`attribution` is mandatory whenever `data` is present. Maya's system prompt
binds rendering to it: no attribution → no equipment claim.

## 4.4 AuthN/Z

* Maya → API: OIDC client-credentials token, audience `maya-automation`, scope
  `equipment.read` / `equipment.search` / `equipment.write`.
* End-user identity is forwarded as `X-Maya-Actor` and recorded on the run; the
  API enforces per-actor quotas (default 20 SIS lookups/hour) so a prompt
  injection cannot turn Maya into a scraper.

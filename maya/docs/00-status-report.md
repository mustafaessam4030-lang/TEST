# Maya Equipment Automation — Status Report

**Project:** Maya (Maia) equipment-data capability for the Mantrac Control Tower
**Source system:** Caterpillar SIS — https://sis2.cat.com/#/
**Branch:** `claude/maya-equipment-automation-yxs92l` · 12 commits · as of 2026-09-20
...
Give Maya a Caterpillar serial number; she returns the equipment data — from our
own store when we have it, from SIS when we don't — with the source, the
timestamp and an automation run id on every answer. Maya orchestrates; she never
drives a browser, holds a credential, or writes SQL.

| Capability | Status |
|---|---|
| Maya chat → tool gateway → Playwright worker → back to Maya | **Working, proven end to end** |
...
**Maya's front end** — her existing chat (v8) is untouched. The capability was
added as a separate layer and patched in at nine hook points by a script that
refuses to run if any anchor has moved.

* `frontend/maya-equipment.js` — the tool layer: five tools, cache-first logic,
...
configured, one question in Maya triggered: learn the layout (17 selectors
...
carries no `data` object at all, so Maya shows the error code and run id and no
equipment values.
...
Open http://127.0.0.1:5173/maya.html and ask:
*"Maya, find equipment data for serial number &lt;a serial that exists&gt;"*
...
| `04-api-and-tools.md` | API surface, Maya's tools, envelopes, authz |
...
| `09-maya-integration.md` | exactly what was added to Maia |
...
|---|---|
| `22056a0` | The platform: gateway, decision flow, adapters, data layer, Maya's tool layer |

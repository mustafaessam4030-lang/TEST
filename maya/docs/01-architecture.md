...
Maya is an LLM **orchestrator**, not an operator. It never touches a browser, a
...
```
  ...
  Teams / Web ───────►│  API Gateway (WAF, mTLS)  │
   ...
   └──────┬──────┘        └────────┬────────┘      └──────────────────┘
          ...
          │                        │
   ┌──────▼────────────────────────▼───────────────────────────────┐
   │ Snowflake (MAYA_PROD)   Vault/KeyVault   OTel Collector       │
...
chat experience; on timeout the API returns `202 + automation_run_id` and Maya
...
|---|---|---|
| `maya-agent` | NL understanding, serial extraction, tool choice, phrasing | Invent field values, retry logic, credentials |
| `maya-api` | Decision flow, freshness, validation, persistence, audit | Free-form navigation |

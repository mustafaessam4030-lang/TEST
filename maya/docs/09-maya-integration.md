# 09 — What Was Added to Maya (and how to switch it on)

Maya was **not** rebuilt. Her NLU, intent classifier, retrieval corpus, planner,
...
| `frontend/maya-equipment.js` | the new tool layer (standalone, reviewable) |
...
half-patched file. Rebuild any time with `make build-maya`.
...
1. Deploy the API (`docker compose up maya-api`, or your cluster).
...
4. Set `MAYA_ALLOW_LIVE_AUTOMATION=true` (off by default so nothing touches a
...
Until step 5 is done, the layer reports `NOT_CONFIGURED` and Maya says the

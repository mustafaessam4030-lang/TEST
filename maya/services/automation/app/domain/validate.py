"""Validation gate. Nothing reaches Maya or the warehouse without passing here."""
...
REQUIRED_FIELDS = ("serial_number", "equipment_model", "source_system", "retrieved_at")
...
def validate_equipment_data(record: EquipmentRecord, *, min_score: float = 0.5) -> EquipmentRecord:
    ...
    if record.build_date:
        if not _ISO_PARTIAL.match(record.build_date):
            violations.append("build_date_not_iso")
        else:
            year = int(record.build_date[:4])
            current_year = datetime.now(timezone.utc).year
            if year < 1925 or year > current_year + 1:
                violations.append(f"build_date_out_of_range:{year}")
